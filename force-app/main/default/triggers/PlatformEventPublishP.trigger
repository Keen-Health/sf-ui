trigger PlatformEventPublishP on Member_s_pharmacy__c(after insert, after delete) {
    
    If(trigger.isAfter){
    
        List<newRelatedListRecordCreated__e> publishEvents = new List<newRelatedListRecordCreated__e>();
        if(trigger.isInsert){
            for(Member_s_pharmacy__c a : Trigger.new){
        ...
        if(trigger.isDelete){
            for(Member_s_pharmacy__c a : Trigger.old){
                newRelatedListRecordCreated__e eve = new newRelatedListRecordCreated__e();
                //eve.recordId__c= a.Id;
