trigger PlatformEventPublishORP on Outbound_Referral_Practice__c(after insert, after delete) {
    
    If(trigger.isAfter){
    
        List<newRelatedListRecordCreated__e> publishEvents = new List<newRelatedListRecordCreated__e>();
        if(trigger.isInsert){
            for(Outbound_Referral_Practice__c a : Trigger.new){
        ...
        if(trigger.isDelete){
            for(Outbound_Referral_Practice__c a : Trigger.old){
                newRelatedListRecordCreated__e eve = new newRelatedListRecordCreated__e();
               // eve.recordId__c= a.Id;
